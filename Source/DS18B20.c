...
{    
    ...
    Delay_10us();
    DS18B20_Int_Write_Byte(0xcc);   //跳过ROM操作命令
    DS18B20_Int_Write_Byte(0x44);     //发送启动温度转换命令
    if(!DS18B20_Int_init())
    {
        return 0xffff;
    }     
    Delay_10us();
    DS18B20_Int_Write_Byte(0xcc);    //跳过ROM操作命令
    DS18B20_Int_Write_Byte(0xbe);      //发送读温度寄存器命令
    a = DS18B20_Int_Read_Byte();    //先读低八位
    b = DS18B20_Int_Read_Byte();      //再读高八位
    t = b;        
    t <<= 8;      //左移八位
...
void DS18B20_Int_Write_Byte(uint8_t dat)
{
    ...
    for(i = 0; i < 8; i++)
    {
        DQ_INT = 0;  //每写入一位数据之前先把总线拉低1us
        _nop_();
        DQ_INT = dat & 0x01;    //取最低位写入
...
{    
    ...
    Delay_10us();
    DS18B20_Ext_Write_Byte(0xcc);   //跳过ROM操作命令
    DS18B20_Ext_Write_Byte(0x44);     //发送启动温度转换命令
    if(!DS18B20_Ext_init())
    {
        return 0xffff;
    }     
    Delay_10us();
    DS18B20_Ext_Write_Byte(0xcc);    //跳过ROM操作命令
    DS18B20_Ext_Write_Byte(0xbe);      //发送读温度寄存器命令
    a = DS18B20_Ext_Read_Byte();    //先读低八位
    b = DS18B20_Ext_Read_Byte();      //再读高八位
    t = b;        
    t <<= 8;      //左移八位
    t = t|a;      //t为16位的数，使高八位为b的值，低八位为a的值  
...
void DS18B20_Ext_Write_Byte(uint8_t dat)
{
    ...
    for(i = 0; i < 8; i++)
    {
        DQ_EXT = 0;  //每写入一位数据之前先把总线拉低1us
        _nop_();
        DQ_EXT = dat & 0x01;    //取最低位写入
        Delay_x_10us(7);    //延时70us，持续时间最少60us
        DQ_EXT = 1;   //然后释放总线
        dat = dat >> 1;    //从低位开始写
    }
